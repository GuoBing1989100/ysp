...
# 新增：统一的 ck_guard_time 生成函数
def make_guard_time(timestamp: int, guid: str) -> str:
    """
    生成 ck_guard_time：timestamp + guid 做校验和，生成 66 位 hex 字符串
    """
    base = f"{timestamp}:{guid}"
    sig = 0
    for b in base.encode("utf-8"):
        sig = (sig * 131 + b) & 0xFFFFFFFF
    # 混入 timestamp
    sig ^= timestamp & 0xFFFFFFFF
    # 生成 66 位 hex
    hex_str = f"{sig:08X}"
    while len(hex_str) < 66:
        hex_str += f"{(sig ^ len(hex_str)) & 0xFFFFFFFF:08X}"
    return hex_str[:66]
...
def ckey42(Platform, Timestamp, Sdtfrom="fcgo", vid="600002264", guid=None, appVer="V8.22.1035.3031"):
    ...
    # 使用统一的 ck_guard_time 生成算法
    ck_guard_time = make_guard_time(Timestamp, guid)
    ...
    data = {
        ...
        "randFlag": create_str_data(base64.b64encode(os.urandom(18)).decode()),
        ...
        "ck_guard_time": create_str_data(ck_guard_time),  # 使用新算法生成
    }
...
def ysp(cnlid: str, livepid: str, defn: str = "auto"):
    try:
        ...
        Cnlid = params['cnlid']
        StaGuid = RandomHexStr(32)  # 统一的 guid
        sdtfrom = 'dcgh'

        # 生成统一的 ck_guard_time（和 ckey42 保持一致）
        ck_guard_time = make_guard_time(Timestamp, StaGuid)

        ckey = ckey42(Platform, Timestamp, sdtfrom, Cnlid, StaGuid, appVer)
        params.update({
            "cKey": ckey,
            "ck_guard_time": ck_guard_time  # m3u8 链接带上相同的参数
        })
